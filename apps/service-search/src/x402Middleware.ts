/**
 * Reusable x402 middleware for Express services.
 * Usage:
 *   app.post('/endpoint', x402Middleware({ price, recipientAddress }), handler)
 *
 * Flow:
 *   1. Request arrives without X-Payment → respond 402 with payment instructions
 *   2. Request arrives with X-Payment: <txHash> → verify on Stellar Horizon → next()
 */
...
export function x402Middleware(opts: X402Options) {
  return async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    // Replay protection (in-memory, resets on restart — acceptable for demo)
